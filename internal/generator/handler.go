...
func (g *HandlerGenerator) generateHandler(outputDir string) (string, error) {
	...
	b.WriteString(fmt.Sprintf("\t\tr.Post(\"/\", h.Create)\n"))
	b.WriteString(fmt.Sprintf("\t\tr.Get(\"/{id}\", h.GetByID)\n"))
	b.WriteString(fmt.Sprintf("\t\tr.Put(\"/{id}\", h.Update)\n"))
	b.WriteString(fmt.Sprintf("\t\tr.Delete(\"/{id}\", h.Delete)\n"))
	b.WriteString("\t})\n")
	b.WriteString("}\n\n")
